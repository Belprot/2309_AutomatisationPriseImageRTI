/* ************************************************************************** */
/** gestion SPI via machine d'état
 *
 *  @Company
 *    ETML-ES - SCA
 *
 *  @File Name
 *    Mc32Ex8_2_spi_sm.c
 *
 *  @Summary
 *    gestion SPI via machine d'état
 *
 *  @Description
 *    gestion SPI via machine d'état
 *    Testé pour fonctionner avec LM70 sur SPI1
 * 
 *   Création 14.03.2017 SCA
 *  
*/

#include "Mc32_spi_sm.h"
#include "system/clk/sys_clk.h" // pour SYS_CLK_PeripheralFrequencyGet()
#include "system/int/sys_int.h"
#include "peripheral\SPI\plib_spi.h"


#define DUMMY_BYTE  0x81   


#define SPI_USED (SPI_ID_1)

SPI_STATES spiState = SPI_STATE_UNINITIALIZED;


//Tableau permettant d'inverser MSB et LSB écriture SPI
const unsigned char bitReverseTable256_A[] =
{

};



//fonction à appeler 1x au démarrage pour init.
//code repris de la génération du driver par Hamony 1.08
void SPI_Init(void)
{
    // *** init du périph. SPI ***
    PLIB_SPI_Disable(SPI_USED);

    PLIB_SPI_BufferClear(SPI_USED);
    PLIB_SPI_StopInIdleDisable(SPI_USED);
//    PLIB_SPI_PinEnable(SPI_USED, SPI_PIN_DATA_OUT);
    PLIB_SPI_CommunicationWidthSelect(SPI_USED, SPI_COMMUNICATION_WIDTH_8BITS);
//    PLIB_SPI_BaudRateSet(SPI_USED, SYS_CLK_PeripheralFrequencyGet(CLK_BUS_PERIPHERAL_1), 1000000);
    // Config polarité traitement des signaux SPI
    // pour input à confirmer
    // Polarité clock OK
    // Phase output à confirmer
//    PLIB_SPI_InputSamplePhaseSelect(SPI_USED, SPI_INPUT_SAMPLING_PHASE_IN_MIDDLE );
//    PLIB_SPI_ClockPolaritySelect(SPI_USED, SPI_CLOCK_POLARITY_IDLE_LOW);
//    PLIB_SPI_OutputDataPhaseSelect(SPI_USED, SPI_OUTPUT_DATA_PHASE_ON_IDLE_TO_ACTIVE_CLOCK);
//    PLIB_SPI_MasterEnable(SPI_USED);
//    PLIB_SPI_FramedCommunicationDisable(SPI_USED);
    PLIB_SPI_FIFOEnable(SPI_USED);     // Enhenced buffer mode

    PLIB_SPI_Enable(SPI_USED);

   
   // action de configuration
    if (SPI_GetState() == SPI_STATE_IDLE)               
    {
        SPI_StartRead(4);               
    }
    else if (SPI_GetState() == SPI_STATE_IDLE_READ_DATA_AVAILABLE)               
    {
        SPI_ReadByte();
        SPI_ReadByte();
        SPI_ReadByte();
        SPI_ReadByte();
    }
 
}

//Ecriture.
//Comme le SPI est obligatoirement full-duplex,
//les données reçues ne seront pas traitées
void SPI_StartWrite(uint32_t nBytes, uint8_t* pBytesToWrite)
{
    uint8_t i;
    
    for(i=0; i<nBytes; i++)
    {
        do{
            
        }while(PLIB_SPI_TransmitBufferIsFull(SPI_USED));
        
        *pBytesToWrite = bitReverseTable256_A[*pBytesToWrite];
        PLIB_SPI_BufferWrite(SPI_USED, *pBytesToWrite);
        pBytesToWrite++;
    }
    // Updates SM
    spiState = SPI_STATE_BUSY_WRITE;
}

//Lecture/écriture.
//Comme le SPI est obligatoirement full-duplex,
//des données sont reçues simultanément à l'envoi
void SPI_StartReadWrite(uint32_t nBytes, uint8_t* pBytesToWrite)
{
     // Variables locales
    uint8_t i;
    
    // Ecriture des bytes
    for(i=0; i<nBytes; i++)
    {
        
        PLIB_SPI_BufferWrite(SPI_USED, *pBytesToWrite);
        pBytesToWrite++;
    }
    // Updates SM
    spiState = SPI_STATE_BUSY_READ_WRITE;
}

//Lecture.
//Comme le SPI est obligatoirement full-duplex,
//il faut envoyer des données bidons pour faire une lecture
void SPI_StartRead(uint32_t nBytes)
{
    // Variables locales
    uint8_t i;
    
    // Ecriture des bytes
    for(i=0; i<nBytes; i++)
    {
        PLIB_SPI_BufferWrite(SPI_USED, DUMMY_BYTE);
    }
    // Updates SM
    spiState = SPI_STATE_BUSY_READ;    
}

//pour obtenir l'état interne de la SM spi
SPI_STATES SPI_GetState (void)
{
    return spiState;
}

//lecture d'un byte dans buffer réception
uint8_t SPI_ReadByte(void)
{
    uint8_t data;
    
    data =  PLIB_SPI_BufferRead(SPI_USED);
    if (PLIB_SPI_ReceiverFIFOIsEmpty(SPI_USED))
    {
        // Updates SM
        spiState = SPI_STATE_IDLE;   
    }
    return (data);
}

// SPI State machine
void SPI_DoTasks(void){
    
    switch(spiState)
    {
        case SPI_STATE_UNINITIALIZED:
            SPI_Init();
            spiState = SPI_STATE_IDLE;
            break;
        case SPI_STATE_IDLE:
            break;
        case SPI_STATE_IDLE_READ_DATA_AVAILABLE:
            break;
        case SPI_STATE_BUSY_WRITE:
            if(!PLIB_SPI_IsBusy(SPI_USED))
            {
               spiState = SPI_STATE_IDLE;
            }
            break;
        case SPI_STATE_BUSY_READ_WRITE:
            if(!PLIB_SPI_IsBusy(SPI_USED) && !PLIB_SPI_ReceiverFIFOIsEmpty(SPI_USED))
            {
                spiState = SPI_STATE_IDLE_READ_DATA_AVAILABLE;
            }
            break;
        case SPI_STATE_BUSY_READ:
            if(!PLIB_SPI_IsBusy(SPI_USED) && !PLIB_SPI_ReceiverFIFOIsEmpty(SPI_USED))
            {
                spiState = SPI_STATE_IDLE_READ_DATA_AVAILABLE;
            }
            break;
        default:
            break;
    }
}


/* *****************************************************************************
 End of File
 */
