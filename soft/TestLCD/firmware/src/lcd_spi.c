...
{
	spi_write(0x5F);				//Send 5 synchronisation bits, RS = 1, R/W = 0
	spi_write(data & 0x0F);		//send lower data bits
	spi_write((data>>4) & 0x0F);	//send higher data bits
}
